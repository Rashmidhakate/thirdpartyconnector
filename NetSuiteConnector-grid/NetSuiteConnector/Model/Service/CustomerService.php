...
use Zoku\NetSuiteConnector\Api\CustomerServiceInterface as CustomerServiceInterface;
...
class CustomerService implements CustomerServiceInterface
{
    ...
    {
        if (!$this->baseService->getModuleEnable() || !$this->baseService->getLogConfig()) {
            return false;
        }
     ...
     * @throws Exception
     */
    public function saveCustomerLoyaltyLog($requestPayload, $responsePayload)
    {
        if (!$this->baseService->getModuleEnable() || !$this->baseService->getLogConfig()) {
            return false;
        }
        ...
        if (!empty($request['body'] && !empty($request['body']['fields']))) {
            ...
            foreach ($customerList as $key => $value) {
                $customerArray[] = [
                    ...
                    'flag' => $customerList[$key]['flag'],
                    'customer_netsuite_id' => $customerList[$key]['customer_netsuite_id'],
                    'request_payload' => $customerList[$key]['request_payload'],
                    'response_payload' => $customerList[$key]['response_payload']
                ];
            }
            try {
                $tableName = $this->resource->getTableName($table);
